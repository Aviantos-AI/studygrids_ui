...
// ✅ Helper Function for Enrollment Dialog
void _openEnrollmentDialog(BuildContext context) {
  showDialog(
    context: context,
    builder: (ctx) => const EnrollmentFormDialog(),
...
// ✅ Helper Function for Mailchimp
Future<void> _launchMailchimpUrl(BuildContext context) async {
  final Uri uri = Uri.parse(mailchimpUrl);
  if (kIsWeb) {
...
// ✅ Converted to StatefulWidget for Mixin
class AgenticDevelopersDetailPage extends StatefulWidget {
  const AgenticDevelopersDetailPage({super.key});

  @override
  State<AgenticDevelopersDetailPage> createState() =>
      _AgenticDevelopersDetailPageState();
}

// ✅ Added Mixin
class _AgenticDevelopersDetailPageState extends State<AgenticDevelopersDetailPage>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ✅ Wrapped in Stack for Sticky Bottom Bar
      body: Stack(
        children: [
          SingleChildScrollView(
            // ✅ Connect Mixin Controller
            ...
            child: Column(
              ...
              children: [
                ...
                // ✅ Extra padding for sticky bar space on mobile
...
class _HeroSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const devGreen = Color(0xFF00C08A);
    const devorange = Color(0xFFFF7E21);
    final isWide = MediaQuery.of(context).size.width >= 900;

    // ✅ FIX: Use 'Flex' logic carefully.
    // If not wide (mobile), use Column directly without Flexible/Expanded
    // because parent is scrollable (unbounded height).
    if (!isWide) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          // RIGHT VISUAL CARD (Mobile Bottom)
          // Removed Expanded here to fix the unbounded height error
          SizedBox(
            height: 220, // Fixed height for mobile image container
            width: double.infinity,
            child: ClipRRect(
              borderRadius: BorderRadius.circular(24),
              child: _buildHeroImage(devGreen), // Extracted image widget
            ),
          ),
        ],
      );
      ...
      children: [
        ...
        Expanded(
          ...
          child: SizedBox(
            ...
            child: ClipRRect(
              borderRadius: BorderRadius.circular(24),
              child: _buildHeroImage(devGreen),
            ),
          ),
        ),
      ],
  ...
  // ✅ Helper to build the image stack (avoids code duplication)
  Widget _buildHeroImage(Color devGreen) {
    return Image.asset(
      'lib/assets/images/agentic_ai.jpeg',
      fit: BoxFit.cover,
      errorBuilder: (context, error, stackTrace) {
